...
- OSCタイトル・パレット／動的色制御・色スタック、OSC 8リンク、タブ色・進捗表示、明示許可・サイズ制限付きクリップボードコピーと、表示条件・優先度対応のデスクトップ通知、URL自動検出、作業ディレクトリ・リモートホスト・終了状態付きコマンドマーカー・プロンプト移動を行うシェル連携（[対応表](docs/osc-support.md)）
