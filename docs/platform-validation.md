...
both X11 (Openbox on Xvfb using Mesa's Lavapipe driver) and Wayland (Weston
nested under Xvfb). The GUI smoke command exits only after creating the window,
GPUI renderer, IME handler, and initial shell session, and painting a frame.
